    ...
    public void performAction(String actionMessage) throws FileNotFoundException {
        try {
            ...
            if (stra[0].equals("addPage")) {
                ...
                // TODO - fix this Hack ASAP!
                for (int j = 0; j < res.size(); j++) {
                    if (str.length == 1) {
                        cool.insert(
                                new SearchResult(
                                        res.l.get(j).element,
                                        res.l.get(j).element.getRelevanceofPage(str, false)));
                    } 
                    else if (str.length == 2) {
                        for (int x = 0;
                                x < res.l.get(j).element.pin.wget_data(str[0]).element.pos.size;
                                x++) {
                            int k =
                                    res.l
                                            .get(j)
